...
export const LandingPage: React.FC = () => {
  ...
  const { isFirebaseReady } = useAppContext();
...
[Thông tin liên hệ (Số điện thoại, email)]`,
  ...
  const teamImageMapping: { [key: string]: string } = {
    'Fukuoka Kamikaze': 'assets/phoenix.png',
    'Magical Feet': 'assets/dragon.png',
    'V - All Star': 'assets/tiger.png',
    'Không thể cản phá': 'assets/turtle.png',
  };

  const teamData =
    tournament?.teams.map((team) => ({
      ...team,
      image: teamImageMapping[team.name] || 'assets/VFLogo-fix.png', // fallback image
    })) || [];
    ...
    <>
      ...
      >
        <div className="min-h-screen bg-black/40 backdrop-blur-[2px] p-4 sm:p-6 flex flex-col relative overflow-hidden shine-effect">
          <div
            ref={contentRef}
            className="flex flex-col flex-grow transition-transform duration-500 ease-out relative z-10"
          >
            {/* Header */}
            <header
            ...
            >
              <div className="flex flex-col items-center text-center gap-2 w-full md:w-1/3 order-2 md:order-1">
                ...
                <img src="assets/vnext/vnext-logo-horizontal.svg" alt="VNEXT" className="w-28 h-auto dark:hidden" />
                <img
                  src="assets/vnext/vnext-logo-horizontal-light.svg"
                  alt="VNEXT"
              ...
              <div className="text-center w-full md:w-1/3 order-1 md:order-2 flex flex-col items-center gap-1">
                <span className="font-bold bg-card text-black px-3 py-1 rounded-md text-sm">VNEXT JAPAN株式会社</span>
                <p>GIẢI BÓNG ĐÁ NỘI BỘ</p>
                <p>Chào mừng kỉ niệm 9 năm thành lập VNEXT JAPAN</p>
              </div>
            ...
            <main className="flex-grow flex flex-col items-center justify-center text-center my-8">
              <div className="will-animate" style={{ animation: 'fadeInUp 0.6s ease-out 0.2s forwards' }}>
                <div className="relative bg-foreground py-2 sm:py-4 px-8 sm:px-24 border-4 sm:border-8 border-primary/30 -skew-x-12 shadow-orange-xl transition-all duration-300 hover:scale-[1.03] hover:shadow-orange-lg">
                  <div className="absolute -top-3 -left-10 sm:-top-4 sm:-left-12 w-6 h-6 sm:w-8 sm:h-8 bg-primary"></div>
                  <div className="absolute -bottom-3 -right-10 sm:-bottom-4 sm:-right-12 w-6 h-6 sm:w-8 sm:h-8 bg-primary"></div>
                  <h1 className="text-3xl sm:text-6xl font-black tracking-wider skew-x-12">VNEXT JAPAN</h1>
                  <h2 className="text-4xl sm:text-7xl font-black text-vnext-deep dark:text-primary tracking-wider skew-x-12">
                    OPEN CUP
                  </h2>
                </div>
                <div className="mt-8">
                  <h3 className="text-4xl sm:text-6xl font-extrabold tracking-tight">TỨ HÙNG TRANH ĐẤU</h3>
                  <p className="mt-2 text-lg sm:text-2xl font-semibold text-foreground">
                    Khát vọng bứt phá - Kết nối đam mê
                  </p>
                </div>
              </div>
              ...
              >
                {teamData.map((team) => (
                  ...
                  >
                    <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-full bg-primary p-2 transform transition-all duration-300 group-hover:scale-110 group-hover:shadow-orange-xl group-hover:[filter:drop-shadow(0_0_15px_rgba(255,193,7,0.6))]">
                      <div className="w-full h-full rounded-full bg-card/20 flex items-center justify-center">
                        <img src={team.image} alt={team.name} className="w-[170%] h-[170%] object-contain" />
                      </div>
                    </div>
                    <p className="font-bold text-lg bg-black/30 px-3 py-1 rounded-md transition-colors duration-300 group-hover:bg-black/60">
                      {team.name}
                    </p>
                  </div>
                ))}
              ...
              <div className="mt-12 will-animate" style={{ animation: 'fadeInUp 0.6s ease-out 0.6s forwards' }}>
                {diamondSponsors.length > 0 && (
                  <div className="mb-8">
                    <h4 className="font-extrabold text-2xl border-b-2 border-primary/30 text-primary inline-block px-4 pb-1 mb-4 text-center">
                    ...
                    <div className="flex flex-wrap justify-center items-center gap-4 text-sm sm:text-base font-semibold whitespace-pre-line text-center">
                      {diamondSponsors.map((sponsor) => (
                        <button
                          ...
                          className="bg-gradient-to-br from-primary to-primary text-white border-2 border-primary/30 p-4 rounded-xl shadow-orange-lg transition-all duration-300 hover:scale-105 hover:shadow-orange-lg text-center"
                        >
                          {sponsor}
                        </button>
                      ))}
                ...
                {platinumSponsors.length > 0 && (
                  <div className="mb-8">
                    <h4 className="font-extrabold text-xl border-b-2 border-border text-foreground inline-block px-4 pb-1 mb-4 text-center">
                    ...
                    <div className="flex flex-wrap justify-center items-center gap-4 text-sm sm:text-base font-bold whitespace-pre-line text-center">
                      {platinumSponsors.map((sponsor) => (
                        <button
                          key={sponsor}
                          onClick={() => handleSponsorClick(sponsor)}
                          className="bg-gradient-to-br from-muted to-muted text-black border-2 border-border p-3 rounded-lg shadow-orange-lg transition-all duration-300 hover:scale-105 hover:shadow-orange-lg text-center"
                        >
                          {sponsor}
                        </button>
                      ))}
                ...
                {goldSponsors.length > 0 && (
                  <div className="mb-8">
                    <h4 className="font-extrabold text-lg border-b-2 border-warning/40 text-vnext-amber inline-block px-4 pb-1 mb-4 text-center">
                      NHÀ TÀI TRỢ VÀNG
                    </h4>
                    <div className="flex flex-wrap justify-center items-center gap-4 text-sm sm:text-base font-semibold whitespace-pre-line text-center">
                      {goldSponsors.map((sponsor) => (
                        <button
                          key={sponsor}
                          onClick={() => handleSponsorClick(sponsor)}
                          className="bg-gradient-to-br from-primary to-primary text-black border-2 border-warning/40 p-2 rounded-md shadow-orange-md transition-all duration-300 hover:scale-105 hover:shadow-orange-lg text-center"
                        >
                          {sponsor}
                        </button>
                      ))}
            ...
            >
              <button
                ...
                className="bg-primary text-black p-4 rounded-lg shadow-orange-lg flex flex-col justify-center text-center md:w-auto h-full transition-all duration-300 hover:scale-105 hover:shadow-orange-lg"
              >
            ...
            {/* Navigation Button */}
            <div className="text-center mt-8 will-animate" style={{ animation: 'fadeInUp 0.6s ease-out 1s forwards' }}>
              <Link
                to="/home"
                className="inline-block bg-card text-vnext-deep dark:text-primary font-bold py-3 px-8 rounded-full shadow-orange-lg hover:bg-muted transition-all duration-300 text-lg transform hover:scale-105 hover:shadow-orange-xl hover:shadow-white/30"
              >
      ...
      {isLocationModalOpen && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-foreground bg-opacity-60 backdrop-blur-sm p-4"
          onClick={() => setIsLocationModalOpen(false)}
          style={{ animation: 'fadeIn 0.3s ease-out forwards' }}
        >
          <div
            className="bg-card text-black rounded-lg shadow-orange-xl max-w-lg w-full m-4 p-6 relative max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-4 text-foreground">Thông Tin Địa Điểm & Di Chuyển</h3>
            <div className="space-y-4 text-left text-foreground">
              ...
              <div>
                <p className="font-semibold text-foreground">Cách di chuyển:</p>
                <ul className="list-decimal list-inside space-y-1 mt-1 pl-2">
                  ...
                  <li>
                    ...
                    <span className="font-mono bg-muted px-1 rounded">평２３</span>.
                  </li>
                  <li>
                    Từ ga Kameido (Line Chūō-Sōbu) đi xe buýt{' '}
                    <span className="font-mono bg-muted px-1 rounded">錦２５</span> hoặc{' '}
                    <span className="font-mono bg-muted px-1 rounded">亀２６</span>.
                  </li>
                </ul>
              </div>
            </div>
            <div className="mt-6 space-y-4 text-center">
              <div>
                <p className="font-semibold text-foreground mb-2">Lối vào tại ô vuông được đánh dấu</p>
                <img
                  src="assets/location.png"
                  alt="Sơ đồ lối vào sân"
                  className="w-full h-auto rounded-lg border border-border shadow-orange-sm"
                />
              </div>
              <a
                ...
                className="inline-flex items-center gap-2 bg-primary text-white font-bold py-2 px-6 rounded-full shadow-orange-lg hover:bg-primary transition-all duration-300 transform hover:scale-105"
              >
      ...
      {isThankYouModalOpen && (
        <div
          className="fixed inset-0 z-[60] flex items-center justify-center bg-foreground bg-opacity-70 backdrop-blur-sm p-4"
        ...
        >
          ...
          >
            ...
            <img
              ...
              className="w-full h-auto rounded-lg shadow-orange-xl"
            />
          </div>
        </div>
      )}

...
export const ManageTournamentModal: React.FC<ManageTournamentModalProps> = ({
  isOpen,
  onClose,
  tournament: initialTournament,
  allPlayers,
  availablePlayersForLookup,
}) => {
  ...
  const handleAddMemberToTeam = (teamId: string) => {
    ...
    setTeams((prev) =>
      prev.map((t) => (t.id === teamId ? { ...t, members: [...t.members, { playerId: playerIdToAdd }] } : t)),
    );
  ...
  const handleRemoveMemberFromTeam = (teamId: string, playerId: string) => {
    setTeams((prev) =>
      prev.map((t) => (t.id === teamId ? { ...t, members: t.members.filter((m) => m.playerId !== playerId) } : t)),
    );
  };
  ...
  const handleAddPlayer = async () => {
    ...
    if (jersey !== 0 && allPlayers.some((p) => p.jerseyNumber === jersey)) {
    ...
    return (
      <Modal isOpen={isOpen} onClose={onClose} title={translate('manageTournament.migration.title')} size="lg">
        <div className="text-center p-4">
          <InformationCircleIcon className="w-12 h-12 mx-auto text-vnext-amber mb-4" />
          <h3 className="text-xl font-bold text-foreground">{translate('manageTournament.legacyWarning.title')}</h3>
          <p className="mt-2 text-muted-foreground">{translate('manageTournament.migration.body')}</p>
          <Button onClick={handleMigrate} disabled={isMigrating} size="lg" className="mt-6" variant="outline">
  ...
  const renderPlayersTab = () => (
    <div className="space-y-4">
      <div className="p-3 bg-primary/10 dark:bg-primary/40 text-vnext-deep dark:text-primary rounded-lg text-sm flex items-start gap-2">
        <InformationCircleIcon className="w-5 h-5 mt-0.5 flex-shrink-0" />
        <span>{translate('manageTournament.players.liveSaveInfo')}</span>
      </div>
      <div className="p-3 bg-muted/50 dark:bg-background/50 rounded-lg">
        <h4 className="font-semibold mb-2 text-foreground">{translate('manageTournament.button.addPlayer')}</h4>
        <div className="flex items-center space-x-2">
          ...
          />
          <Button type="button" onClick={handleAddPlayer} variant="soft">
      ...
      <div className="space-y-2">
        {allPlayers.map((player) => (
          <div key={player.id} className="flex items-center justify-between p-2 bg-card /50 rounded-md gap-2">
            <input
              type="text"
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center space-x-2">
        <input
          ...
          placeholder={translate('manageTournament.teamName.placeholder')}
          className={inputClasses}
        />
        <Button type="button" onClick={handleAddTeam} variant="soft">
          <PlusIcon className="w-5 h-5 mr-1" />
          {translate('manageTournament.button.addTeam')}
        </Button>
      </div>
      <div className="space-y-4">
        {teams.map((team) => {
          ...
          return (
            <div key={team.id} className="p-3 bg-background rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-3 flex-grow">
                  <input
                    type="color"
                    value={team.color || 'hsl(var(--muted-foreground))'}
                    onChange={(e) =>
                      setTeams((prev) => prev.map((t) => (t.id === team.id ? { ...t, color: e.target.value } : t)))
                    }
                    className="w-8 h-8 p-1 bg-card border border-border rounded-md cursor-pointer"
                  ...
                  <input
                    ...
                    className="font-bold text-lg bg-transparent border-none focus:ring-0 p-1 w-full text-foreground"
                  />
                </div>
                <Button type="button" onClick={() => handleRemoveTeam(team.id)} variant="danger" size="sm">
                  <XIcon className="w-4 h-4 mr-1" />
                  {translate('manageTournament.button.removeTeam')}
                </Button>
              </div>
              <div className="space-y-2 mb-2">
                {team.members.map((memberRef) => {
                  ...
                  if (!member)
                    return (
                      <div
                        key={memberRef.playerId}
                        className="flex items-center justify-between text-sm bg-card p-2 rounded text-danger-text italic"
                      >
                        Cầu thủ đã bị xóa
                      </div>
                    );
                  return (
                    <div key={member.id} className="flex items-center justify-between text-sm bg-card p-2 rounded">
                      <span>
                      ...
                      <Button
                        ...
                        className="!p-1"
                      >
                        <XIcon className="w-4 h-4 text-danger-text" />
                      </Button>
                    </div>
                  );
                })}
              ...
              <div className="flex items-center space-x-2 mt-2">
                ...
                <Button type="button" onClick={() => handleAddMemberToTeam(team.id)} size="sm" variant="soft">
                  {translate('manageTournament.button.addMember')}
                </Button>
              </div>
            </div>
          );
        ...
        <div className="border-b border-border mb-4">
          <nav className="-mb-px flex space-x-4" aria-label="Tabs">
            <button
              ...
              className={`${tabButtonBase} ${activeTab === 'teams' ? activeTabClass : inactiveTabClass}`}
            >
              {translate('manageTournament.tab.teams')}
            </button>
            <button
              type="button"
              onClick={() => setActiveTab('players')}
              className={`${tabButtonBase} ${activeTab === 'players' ? activeTabClass : inactiveTabClass}`}
            >
              {translate('manageTournament.tab.players')}
            </button>
          </nav>
        </div>

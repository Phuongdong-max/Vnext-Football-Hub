...
export const TournamentJerseyDrawModal: React.FC<TournamentJerseyDrawModalProps> = ({
  isOpen,
  onClose,
  teams,
  onSave,
  ...
  const getTeamBoxClass = (teamId: string) => {
    ...
    if (!jerseyKey) return 'bg-muted/50 ';
    if (jerseyKey === 'jerseys.vnextPink') return 'bg-primary/10 dark:bg-primary/50 border-primary/30';
    if (jerseyKey === 'jerseys.vnextNew') return 'bg-primary/10 dark:bg-primary/50 border-primary/30';
    if (jerseyKey === 'jerseys.pitch') return 'bg-success/10 dark:bg-success/50 border-success/40';
    return 'bg-muted/50 ';
  };

  const unassignedTeams = shuffledTeams.filter((t) => !assignedJerseys[t.id]);
  const canDraw =
    drawState === 'idle' && unassignedTeams.length > 0 && JERSEYS.length > Object.keys(assignedJerseys).length;
      ...
      <div className="flex flex-col h-[70vh]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {shuffledTeams.map((team) => (
            <div
              key={team.id}
              className={`p-4 rounded-lg text-center border-b-4 transition-all duration-500 ${getTeamBoxClass(team.id)}`}
            >
        ...
        <div className="flex flex-col items-center justify-center my-6 text-center flex-grow">
          <div className="w-64 h-24 bg-card rounded-lg flex items-center justify-center shadow-inner">
            <span className="text-xl font-bold text-primary transition-all duration-100">
              {drawState === 'drawing'
                ? diceDisplayText
                : assignedJerseys[unassignedTeams[0]?.id] || diceDisplayText || '—'}
            </span>
          </div>
          <Button onClick={handleDraw} disabled={!canDraw} size="lg" variant="outline" className="mt-4">
            {drawState === 'drawing' && <LoadingSpinner size="sm" color="text-current" className="mr-2" />}

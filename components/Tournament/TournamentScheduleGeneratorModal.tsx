...
export const TournamentScheduleGeneratorModal: React.FC<TournamentScheduleGeneratorModalProps> = ({
  isOpen,
  onClose,
  teams,
  onSave,
  ...
  const getTeamDisplayInfo = useCallback(
    (teamId: string) => {
      if (teamId.startsWith('TBD-')) {
        return { name: `(${translate('schedule.tbd')})`, color: 'hsl(var(--muted-foreground))' };
      }
      const team = teams.find((t) => t.id === teamId);
      return { name: team?.name || '?', color: team?.color || 'hsl(var(--muted-foreground))' };
    },
    [teams, translate],
  );
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={translate('tournament.generatorModal.title')} size="xl">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {(['single-round-robin', 'double-round-robin', 'league-playoffs'] as ScheduleType[]).map((type) => (
            <Button
              ...
              variant={scheduleType === type ? 'soft' : 'outline'}
            >
        ...
        <div className="text-center mt-4">
          <Button onClick={handleGeneratePreview} size="lg" variant="outline">
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <h3 className="font-semibold text-foreground mb-2 text-center">
            {translate('tournament.generatorModal.schedulePreview')}
          </h3>
          <div className="max-h-[45vh] overflow-y-auto space-y-3 p-2 bg-background /50 rounded-md">
            {previewSchedule ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                {previewSchedule.map((match, index) => {
                  ...
                  return (
                    <div key={match.id} className="p-3 bg-card rounded-lg shadow-orange-sm">
                      <p className="font-bold text-muted-foreground text-xs mb-2">
                        {match.matchLabel || translate('schedule.match', { matchNumber: index + 1 })}
                      </p>
                      <div className="flex items-center justify-center gap-2">
                        <span className="flex items-center gap-2 font-medium text-foreground text-right flex-1 justify-end">
                        ...
                        <span className="flex items-center gap-2 font-medium text-foreground text-left flex-1">
                          <div
              ...
              <p className="text-center text-muted-foreground py-8 italic">
                {translate('tournament.generatorModal.noPreview')}
              </p>
            )}
          </div>
        </div>

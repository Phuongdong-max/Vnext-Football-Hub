      ...
      <div className="space-y-4">
        ...
        <div>
          <h3 className="text-lg font-semibold text-foreground mb-2">
            {translate('goalscorerModal.currentScore')}: {goals.length}
          </h3>
          <div className="space-y-2 max-h-40 overflow-y-auto pr-2 bg-background p-2 rounded-md">
            {goals.length > 0 ? (
              goals.map((goal) => (
                <div
                  key={goal.goalId}
                  className="flex items-center justify-between p-2 bg-card rounded-md shadow-orange-sm"
                >
                  <span className="text-foreground">{goal.scorerName}</span>
              ...
              <p className="text-muted-foreground italic text-center py-4">{translate('goalscorerModal.noGoals')}</p>
            )}
          </div>
        </div>
        ...
        {/* Team Members List */}
        <div>
          <h4 className="font-semibold text-foreground mb-2">{translate('goalscorerModal.addGoalFromTeam')}</h4>
        ...
        {/* Guest Scorer */}
        <div>
          <h4 className="font-semibold text-foreground mb-2">{translate('goalscorerModal.addGuestScorer')}</h4>
          <div className="flex items-center space-x-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') handleAddGuestGoal();
              }}
            ...
            />
            <Button onClick={handleAddGuestGoal} disabled={!guestScorerName.trim()} variant="soft">

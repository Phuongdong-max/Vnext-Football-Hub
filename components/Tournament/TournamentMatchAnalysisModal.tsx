...
const ProbabilityBar: React.FC<{ home: number; away: number; draw: number }> = ({ home, away, draw }) => (
  <div className="w-full flex rounded-full h-3 bg-muted overflow-hidden">
    <div
      className="bg-primary transition-all duration-500"
      style={{ width: `${home}%` }}
      title={`Home: ${home}%`}
    ></div>
    <div className="bg-muted transition-all duration-500" style={{ width: `${draw}%` }} title={`Draw: ${draw}%`}></div>
    <div
...
export const TournamentMatchAnalysisModal: React.FC<TournamentMatchAnalysisModalProps> = ({
  isOpen,
  onClose,
  match,
  teams,
    ...
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center py-12 min-h-[400px]">
          ...
          <p className="mt-4 text-muted-foreground text-center">{translate('tournament.matchAnalysis.loading')}</p>
        </div>
      );
    }
    if (error) {
    ...
    return (
      <div className="space-y-4">
        <div className="text-center p-4 bg-card /50 rounded-lg">
          <h3 className="text-xl font-bold text-foreground">{translate('tournament.matchAnalysis.prediction')}</h3>
          <p className="text-3xl font-black text-primary my-2">{analysis.predictedScore}</p>
          <p className="font-semibold text-muted-foreground">{analysis.matchSummary}</p>
        </div>
        ...
        <Section title={translate('tournament.matchAnalysis.probabilities')}>
          ...
          <div className="flex justify-between text-xs font-medium mt-1">
            <span className="text-vnext-deep dark:text-primary">
              {homeTeam.name}: {analysis.winProbability.home}%
            </span>
            <span className="text-muted-foreground">
              {translate('matchResult.draw')}: {analysis.winProbability.draw}%
            </span>
            <span className="text-danger-text">
              {awayTeam.name}: {analysis.winProbability.away}%
            </span>
          </div>
        </Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Section title={translate('tournament.matchAnalysis.teamAnalysis', { teamName: homeTeam.name })}>
            <h5 className="font-semibold text-foreground">{translate('tournament.matchAnalysis.strengths')}</h5>
            <ul className="list-disc list-inside text-xs">
              {analysis.homeTeamAnalysis.strengths.map((s, i) => (
                <li key={i}>{s}</li>
              ))}
            </ul>
            <h5 className="font-semibold text-foreground mt-2">{translate('tournament.matchAnalysis.weaknesses')}</h5>
            <ul className="list-disc list-inside text-xs">
              {analysis.homeTeamAnalysis.weaknesses.map((w, i) => (
                <li key={i}>{w}</li>
              ))}
            </ul>
            <h5 className="font-semibold text-foreground mt-2">{translate('tournament.matchAnalysis.tactics')}</h5>

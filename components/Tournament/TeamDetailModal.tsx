  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={team.name} size="md">
      <div className="space-y-3">
        <h3 className="text-lg font-semibold text-foreground mb-2">Thành viên đội</h3>
        <ul className="space-y-2 max-h-80 overflow-y-auto">
          {teamMembers.length > 0 ? (
            teamMembers.map((player) => (
              <li key={player.id}>
                <button
                  ...
                  className="w-full flex items-center p-2 rounded-md text-left transition-colors hover:bg-muted/50 dark:hover:bg-card"
                >
                  {player.avatarUrl ? (
                    <img
                    ...
                    <UserCircleIcon className="w-10 h-10 text-muted-foreground mr-3" />
                  )}
                  <div>
            ...
            <p className="text-muted-foreground italic text-center py-4">Chưa có thành viên nào.</p>
          )}
        </ul>
      </div>
    </Modal>

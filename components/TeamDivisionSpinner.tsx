...
const sliceColors = [
  // Thang cam VNEXT + trung tính ấm — không dùng bảng cầu vồng (vnext-ui: chống slop #14)
  '#e96620', // vnext-primary
  '#c2410c', // vnext-deep
  '#d97706', // vnext-amber
  '#f7b478', // vnext-secondary
  '#ca8a04', // vnext-gold
  '#fac99a', // vnext-accent
  '#78716c', // trung tính ấm
  '#a8a29e',
  '#57534e',
];
...
export const TeamDivisionSpinner: React.FC<TeamDivisionSpinnerProps> = ({ players, numberOfTeams, onComplete }) => {
  ...
  const handleSpin = () => {
    ...
    setTimeout(() => {
      setAnnouncement(
        translate('teamDivider.spinner.selected', { playerName: selectedPlayer.name, playerSeed: selectedPlayer.seed }),
      );
        ...
        setTeams((prevTeams) => {
          const newTeams = prevTeams.map((team) => {
            if (team.id === targetTeam.id) {
              const newPlayers = [...team.players, selectedPlayer].sort(
                (a, b) => seedValues[b.seed] - seedValues[a.seed],
              );
              return {
                ...team,
                players: newPlayers,
                totalSeedValue:
                  team.totalSeedValue + (selectedPlayer.seed === 'GK' ? 0 : seedValues[selectedPlayer.seed]),
  ...
  return (
    <div className="flex flex-col items-center justify-start p-4 min-h-[80vh] bg-background dark:bg-background/50 rounded-lg">
      {flyingPlayer && (
        <div
      ...
      <h1 className="text-3xl font-bold text-foreground mb-2 text-center">{translate('teamDivider.spinner.title')}</h1>
      <p className="text-lg text-muted-foreground h-8 mb-6 text-center transition-all duration-300">{announcement}</p>
      ...
      >
        ...
        >
          <div className="w-full h-full bg-primary shadow-orange-lg" />
        </div>
            ...
            return (
              <div key={`${player.name}-${index}`} id={`wheel-player-wrapper-${player.name}`} style={spokeStyle}>
                <div style={textPositionerStyle}>
                  <span style={textSpanStyle}>{player.name}</span>
                </div>
              </div>
            );
          })}
        ...
        <Button
          ...
          className="w-28 h-28 absolute z-10 rounded-full !p-0 flex items-center justify-center flex-col text-white shadow-orange-lg bg-secondary hover:bg-opacity-90 transition-transform duration-200 active:scale-95 border-4 border-border"
        >
          <PlayIcon className="w-8 h-8" />
          <span className="font-bold text-lg uppercase tracking-wider">
            {translate('teamDivider.spinner.spinButton')}
          </span>
        </Button>
      </div>

      <div
        className="w-full max-w-6xl mx-auto grid gap-4 mt-10"
      ...
      >
        {teams.map((team) => (
          <div
            key={team.id}
            id={`team-box-${team.id}`}
            className="bg-card rounded-lg shadow-orange-md p-4 min-h-[150px] transition-all duration-300 flex flex-col"
          >
            <h3 className="text-lg font-semibold text-primary mb-2 text-center border-b border-border pb-2">
            ...
            <ul className="space-y-1 flex-grow">
              {team.players.map((p) => (
                <li
                  key={`${p.name}-${team.id}`}
                  className="text-sm text-foreground text-center bg-muted/50 p-1.5 rounded-md shadow-orange-sm"
                >

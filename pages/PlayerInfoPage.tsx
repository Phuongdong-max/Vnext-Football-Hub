...
export const PlayerInfoPage: React.FC = () => {
  ...
  const PlayerDetailPlaceholder = () => (
    <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground p-8 bg-muted/50 /50 rounded-lg">
      <UserCircleIcon className="w-24 h-24 text-muted-foreground mb-4" />
      <h3 className="text-xl font-semibold text-foreground">{translate('playerInfo.noPlayerSelected')}</h3>
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-foreground">{translate('playerInfo.title')}</h1>
          ...
          {canEdit && (
            <div className={panelClasses}>
              ...
              <form onSubmit={handleAddPlayer} className="flex flex-col sm:flex-row items-end gap-3">
                ...
                <Button type="submit" className="w-full sm:w-auto" variant="soft">
              ...
              {filteredPlayers.length > 0 ? (
                filteredPlayers.map((player) => (
                  <button
                    ...
                    className={`w-full text-left flex items-center gap-4 p-3 rounded-md transition-colors ${selectedPlayer?.id === player.id ? 'bg-primary/20' : 'hover:bg-muted/50 dark:hover:bg-card'}`}
                  >
                    <span
                      className={`font-mono text-lg w-8 text-center ${selectedPlayer?.id === player.id ? 'text-primary font-bold' : 'text-muted-foreground'}`}
                    >
                      #{player.jerseyNumber}
                    </span>
                    <span
                      className={`font-semibold ${selectedPlayer?.id === player.id ? 'text-primary' : 'text-foreground'}`}
                    >
                      {player.name}
                    </span>
                  </button>
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-foreground">
                        {isEditing ? translate('playerInfo.editing') : selectedPlayer.name}
                      </h2>
                  ...
                  {!isEditing && canEdit && (
                    <Button variant="outline" onClick={() => setIsEditing(true)} className="flex-shrink-0">
                      ...
                      {translate('playerInfo.editDetails')}
                    </Button>
                  )}
                </div>
                ...
                {isEditing ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setEditedPlayer((p) => (p ? { ...p, name: e.target.value } : null))}
                          className={inputClasses}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditedPlayer((p) => (p ? { ...p, jerseyNumber: parseInt(e.target.value) || 0 } : null))
                          }
                          className={inputClasses}
                        />
                      </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">
                        {translate('playerInfo.avatarUrl')}
                      </label>
                      <input
                        type="text"
                        value={editedPlayer?.avatarUrl || ''}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">
                        {translate('playerInfo.bio')}
                      </label>
                      <textarea
                        value={editedPlayer?.bio || ''}
                        onChange={(e) => setEditedPlayer((p) => (p ? { ...p, bio: e.target.value } : null))}
                        placeholder={translate('playerInfo.bioPlaceholder')}
                        rows={4}
                        className={inputClasses}
                      ></textarea>
                    </div>
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-foreground">{translate('playerInfo.bio')}</h3>
                    <p className="mt-1 text-muted-foreground whitespace-pre-wrap bg-muted/50 /50 p-3 rounded-md min-h-[80px]">
                      {selectedPlayer.bio || 'N/A'}
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-border">
                  <div className="space-y-2">
                    ...
                    {Object.keys(defaultSkills).map((key) => (
                      <div key={key}>
                        <label className="flex justify-between items-center text-sm font-medium text-foreground mb-1">
                          <span>{translate(`playerSkills.${key as keyof PlayerSkills}`)}</span>
                          <span className="font-bold text-primary">
                            {editedPlayer?.skills?.[key as keyof PlayerSkills] || 50}
                          </span>
                        </label>
                        <input
                          type="range"
                          min="1"
                          max="99"
                          value={editedPlayer?.skills?.[key as keyof PlayerSkills] || 50}
                          onChange={(e) =>
                            isEditing &&
                            setEditedPlayer((p) =>
                              p ? { ...p, skills: { ...p.skills!, [key]: parseInt(e.target.value) } } : null,
                            )
                          }
                          disabled={!isEditing}
                ...
                {isEditing && canEdit && (
                  <div className="flex justify-between items-center pt-4 border-t border-border">
                    <Button variant="danger" onClick={handleDeletePlayer}>
                      ...
                      {translate('playerInfo.deletePlayer')}
                    </Button>

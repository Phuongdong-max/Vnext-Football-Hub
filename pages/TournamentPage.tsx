...
const EditMatchModal: React.FC<EditMatchModalProps> = ({ isOpen, onClose, match, teams, onSave }) => {
  ...
  const handleSave = async () => {
    if (
      editedMatch.homeTeamId === editedMatch.awayTeamId &&
      editedMatch.homeTeamId !== '' &&
      !editedMatch.homeTeamId.startsWith('TBD-')
    ) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={translate('schedule.editMatchTitle')}>
      <div className="space-y-4">
        <div>
          <label htmlFor="homeTeam" className="block text-sm font-medium text-foreground mb-1">
            {teams.find((t) => t.id === match.homeTeamId)?.name || match.homeTeamId}
          </label>
          <select
            id="homeTeam"
            value={editedMatch.homeTeamId}
            onChange={(e) => setEditedMatch((m) => ({ ...m, homeTeamId: e.target.value }))}
            className={inputClasses}
          >
            <option value={match.homeTeamId.startsWith('TBD-') ? match.homeTeamId : ''}>
              {translate('schedule.tbd')}
            </option>
            {teams.map((t) => (
        ...
        <div>
          <label htmlFor="awayTeam" className="block text-sm font-medium text-foreground mb-1">
            {teams.find((t) => t.id === match.awayTeamId)?.name || match.awayTeamId}
          </label>
          <select
            id="awayTeam"
            value={editedMatch.awayTeamId}
            onChange={(e) => setEditedMatch((m) => ({ ...m, awayTeamId: e.target.value }))}
            className={inputClasses}
          >
            <option value={match.awayTeamId.startsWith('TBD-') ? match.awayTeamId : ''}>
              {translate('schedule.tbd')}
            </option>
            {teams.map((t) => (
              <option key={t.id} value={t.id}>
...
const TeamDisplay = ({
  ...
  if (teamId.startsWith('TBD-')) {
    return (
      <div className="w-full md:w-2/5 text-center font-semibold text-muted-foreground italic">
        {translate('schedule.tbd')}
      </div>
    );
  }
  const team = teams.find((t) => t.id === teamId);
  if (!team) return <div className="w-full md:w-2/5 text-center font-semibold text-foreground">Unknown Team</div>;
  ...
  return (
    <div
      className={`flex items-center justify-center ${textAlign} w-full md:w-2/5 font-semibold text-foreground ${alignClass} gap-3`}
    >
...
export const TournamentPage: React.FC = () => {
  ...
  const [editingMatchInfo, setEditingMatchInfo] = useState<{
    match: TournamentMatch;
    teamType: 'home' | 'away';
  } | null>(null);
  ...
  const topScorers = useMemo(() => {
    ...
    const scorerStats: Record<
      string,
      { name: string; goals: number; teamId: string; isGuest: boolean; jerseyNumber?: number }
    > = {};
    ...
    tournament.schedule.forEach((match) => {
      const processGoals = (goals: Goal[] = [], teamId: string) => {
        goals.forEach((goal) => {
          ...
          if (!scorerStats[key]) {
            const player = goal.scorerId
              ? availablePlayersForTournament.find((p) => p.id === goal.scorerId)
              : undefined;
            scorerStats[key] = {
    ...
    return Object.values(scorerStats)
      .map((stats) => ({ ...stats, teamName: tournament.teams.find((t) => t.id === stats.teamId)?.name || '?' }))
    ...
    const newSchedule = tournament.schedule.map((match) => {
      if (match.id === matchId) {
        ...
        if (teamType === 'home') {
          ...
          updatedMatch.homeTeamScore = goals.length;
        } else {
          updatedMatch.awayTeamGoals = goals;
          updatedMatch.awayTeamScore = goals.length;
        }
        updatedMatch.status =
          updatedMatch.homeTeamScore !== null && updatedMatch.awayTeamScore !== null ? 'finished' : 'scheduled';
        return updatedMatch;
      }
      return match;
    });
  ...
  if (isLoading && !tournament) {
    return (
      <div className="flex justify-center items-center py-10">
        ...
        <p className="ml-4 text-foreground">{translate('tournament.loading')}</p>
      </div>
    );
  }

  const panelClasses = 'bg-card border border-border shadow-orange-sm rounded-xl';
    ...
    if (!tournament) {
      return (
        <div className="text-center py-10">
          <p className="text-muted-foreground">{translate('tournament.noData')}</p>
          {canEdit && (
            <Button
              onClick={() => {
                setModalMode('create');
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <header className="relative text-center">
          <div className="bg-card backdrop-blur-sm p-4 rounded-xl shadow-orange-lg inline-block border border-border">
        ...
        {canEdit && (
          <div className={`${panelClasses} p-3`}>
            <h2 className="text-lg font-semibold mb-2 text-foreground">
              {translate('tournament.generateScheduleSectionTitle')}
            </h2>
              ...
              <button
                ...
                className={`flex items-center gap-2 whitespace-nowrap py-3 px-2 sm:px-4 border-b-2 font-medium text-sm transition-colors duration-200 ${
                  activeTab === tab.id
                    ...
                    : 'border-transparent text-muted-foreground hover:border-border dark:hover:border-border hover:text-foreground'
                }`}
                  ...
                  <table className="min-w-full">
                    <thead className="bg-muted/50 ">
                      <tr>
                        <th
                          scope="col"
                          className="w-1/3 pl-4 pr-3 py-3.5 text-left text-sm font-semibold text-foreground sm:pl-6"
                        >
                          {translate('standingsTable.team')}
                        </th>
                        <th
                          scope="col"
                          className="px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.played')}
                        >
                          {translate('standingsTable.played')}
                        </th>
                        <th
                          scope="col"
                          className="px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.wins')}
                        >
                          {translate('standingsTable.wins')}
                        </th>
                        <th
                          scope="col"
                          className="px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.draws')}
                        >
                          {translate('standingsTable.draws')}
                        </th>
                        <th
                          scope="col"
                          className="px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.losses')}
                        >
                          {translate('standingsTable.losses')}
                        </th>
                        <th
                          scope="col"
                          className="hidden lg:table-cell px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.gf')}
                        >
                          {translate('standingsTable.gf')}
                        </th>
                        <th
                          scope="col"
                          className="hidden lg:table-cell px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.ga')}
                        >
                          {translate('standingsTable.ga')}
                        </th>
                        <th
                          scope="col"
                          className="px-2 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.gd')}
                        >
                          {translate('standingsTable.gd')}
                        </th>
                        <th
                          scope="col"
                          className="px-4 py-3.5 text-center text-sm font-semibold text-foreground"
                          title={translate('standingsTable.points')}
                        >
                          {translate('standingsTable.points')}
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border bg-card">
                      {standings?.map((s, index) => (
                        <tr
                          key={s.teamId}
                          className="hover:bg-muted/50 dark:hover:bg-card/40 transition-colors duration-200"
                        >
                          <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                            <div className="flex items-center">
                              <span className="w-6 text-center mr-3 font-bold text-lg text-muted-foreground">
                                {index + 1}
                              </span>
                              <div
                                style={{ backgroundColor: s.teamColor || 'hsl(var(--muted-foreground))' }}
                                className="w-1.5 h-6 rounded-full mr-4 flex-shrink-0 shadow-orange-sm"
                              ></div>
                              <div className="font-semibold text-base text-foreground">{s.teamName}</div>
                            </div>
                          </td>
                          <td className="whitespace-nowrap px-2 py-4 text-center text-base font-medium text-muted-foreground">
                            {s.played}
                          </td>
                          <td className="whitespace-nowrap px-2 py-4 text-center text-base font-semibold text-success">
                            {s.wins}
                          </td>
                          <td className="whitespace-nowrap px-2 py-4 text-center text-base font-semibold text-vnext-amber">
                            {s.draws}
                          </td>
                          <td className="whitespace-nowrap px-2 py-4 text-center text-base font-semibold text-danger-text">
                            {s.losses}
                          </td>
                          <td className="whitespace-nowrap hidden lg:table-cell px-2 py-4 text-center text-sm text-muted-foreground">
                            {s.goalsFor}
                          </td>
                          <td className="whitespace-nowrap hidden lg:table-cell px-2 py-4 text-center text-sm text-muted-foreground">
                            {s.goalsAgainst}
                          </td>
                          <td className="whitespace-nowrap px-2 py-4 text-center text-base font-bold text-foreground">
                            {s.goalDifference > 0 ? `+${s.goalDifference}` : s.goalDifference}
                          </td>
                          <td className="whitespace-nowrap px-4 py-4 text-center">
                            <span className="inline-block bg-primary text-white text-base font-bold px-3 py-1 rounded-md shadow-orange-md">
                              {s.points}
                            </span>
                          </td>
                        </tr>
            ...
            <section className="space-y-4">
              {schedule?.length > 0 ? (
                        ...
                        <div className="flex items-center gap-1">
                          {!match.homeTeamId.startsWith('TBD-') && !match.awayTeamId.startsWith('TBD-') && (
                            <Button
                              ...
                              onClick={() => setMatchForAnalysis(match)}
                              title="AI Analysis"
                            >
                              <span className="font-bold text-sm bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary">
                                AI
                              </span>
                            </Button>
                          )}
                          ...
                          {canEdit ? (
                            <div className="flex items-center space-x-2">
                              <button
                                ...
                                className="w-20 sm:w-24 text-xl font-bold text-center text-foreground bg-background border border-border rounded-lg p-2 cursor-pointer hover:bg-muted/50 dark:hover:bg-card hover:border-primary/50 transition-all shadow-orange-sm"
                            ...
                            <span className="text-xl font-bold px-3 py-1 text-foreground bg-background rounded-md">
                ...
                <div className={`${panelClasses} p-4`}>
                  <p className="text-muted-foreground text-center py-4">{translate('schedule.noMatches')}</p>
                </div>
              )}
            </section>
          )}
                ...
                <div key={team.id} className={panelClasses}>
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      >
                        <span className="font-bold text-lg bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary">
                          AI
                        </span>
                      </Button>
                    </div>
                    <h4 className="font-semibold text-sm text-foreground mb-1">{translate('teamList.members')}</h4>
                    <ul className="space-y-1">
                          ...
                          <li key={memberRef.playerId} className="p-1 rounded">
                            {player ? (
                              <button
                                ...
                                className="flex items-center text-sm w-full text-left hover:bg-muted/50 dark:hover:bg-card/60 text-foreground p-1 rounded transition-colors duration-150"
                              >
                                <UserCircleIcon className="w-6 h-6 text-muted-foreground mr-2 flex-shrink-0" />
                                <span className="hover:underline">
                                  {player.name} (#{player.jerseyNumber})
                                </span>
                              </button>
                            ) : (
                              <div className="flex items-center text-sm p-1 text-foreground">
                                <UserCircleIcon className="w-6 h-6 text-muted-foreground mr-2 flex-shrink-0" />
                      ...
                      {(!team.members || team.members.length === 0) && (
                        <p className="text-xs text-muted-foreground italic">{translate('teamList.noMembers')}</p>
                      )}
                    </ul>
                  </div>
                </div>
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-grow">
            ...
            <select
              ...
              className="max-w-xs w-full h-10 px-3 rounded-md border border-input bg-card text-foreground text-sm placeholder:text-muted-foreground transition-shadow duration-150 ease-spring focus:outline-none focus:border-primary focus:ring-2 focus:ring-ring/20 disabled:opacity-50"
            >
              {isListLoading ? (
          ...
          {canEdit && (
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                variant="danger"
                disabled={!tournament || allTournaments.length <= 1}
              >
                <TrashIcon className="w-4 h-4 mr-1" />
                {translate('tournament.button.delete')}
              </Button>
              <Button onClick={() => setIsManageModalOpen(true)} size="sm" disabled={!tournament}>
      ...
      {isCreateEditModalOpen && canEdit && (
        <CreateEditTournamentModal
          ...
          mode={modalMode}
          initialName={modalMode === 'edit' ? tournament?.name : ''}
          onSubmit={modalMode === 'create' ? handleCreateTournament : handleEditTournament}
        />
      )}
      ...
      {isGoalscorerModalOpen && editingMatchInfo && tournament && canEdit && (
        <GoalscorerModal
          ...
          allPlayers={availablePlayersForTournament}

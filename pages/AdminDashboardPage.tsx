...
import { PlusCircleIcon, RefreshIcon, ShieldExclamationIcon, ListBulletIcon } from '../components/icons';
import { LoadingSpinner } from '../components/shared/LoadingSpinner';
import { ToggleSwitch } from '../components/shared/ToggleSwitch';
import { EmptyState } from '../components/shared/EmptyState';

export const AdminDashboardPage: React.FC = () => {
  const { currentUser, addToast, refreshLeaderboard, isFirebaseReady, isBettingEnabled, updateAppSettings } =
    useAppContext();
  ...
  const fetchAdminPageData = useCallback(
  ...
  const handleLoadMatchesForFootballDataModal = useCallback(
    async (date: string, leagueCode: string): Promise<FootballMatch[]> => {
      if (!apiAvailable) {
  ...
  const handleUpdateResult = async (roundId: string, winningTeam: MatchResultTeam) => {
    ...
    try {
      ...
      addToast('success.resultUpdatedForRound', 'success', {
        homeTeam: updatedRound.matchDetails.homeTeam,
        awayTeam: updatedRound.matchDetails.awayTeam,
      });
      fetchAdminPageData(true);
      refreshLeaderboard();
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-10 text-foreground">
        <LoadingSpinner size="lg" /> <span className="ml-2">{translate('adminDashboard.loading')}</span>
      </div>
    );
  }

  if (!currentUser || currentUser.role !== 'admin') {
    return <p className="text-center text-danger-text">{translate('adminDashboard.accessDenied')}</p>;
  }

  const openRounds = bettingRounds.filter((r) => r.status === BettingRoundStatus.OPEN);
  const closedRounds = bettingRounds.filter(
    (r) => r.status === BettingRoundStatus.RESULT_UPDATED || r.status === BettingRoundStatus.CLOSED,
  );

  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-foreground">{translate('adminDashboard.title')}</h1>
        <div className="space-x-2">
          <Button
            onClick={() => fetchAdminPageData(true)}
            variant="outline"
            size="sm"
            title={translate('adminDashboard.button.refreshData')}
            aria-label={translate('adminDashboard.button.refreshData')}
            disabled={isDataLoading || isLoading || !isBettingEnabled}
          >
      ...
      <section className="p-4 bg-card rounded-lg shadow-orange-md border border-border">
        <h2 className="text-xl font-semibold text-foreground mb-4">Global App Controls</h2>
        <ToggleSwitch
          id="betting-toggle"
      ...
      {!isBettingEnabled ? (
        <div className="p-3 bg-destructive/10 dark:bg-destructive/20 border border-destructive/40 dark:border-destructive/40 text-sm text-danger-text rounded-md flex items-center">
          <ShieldExclamationIcon className="w-5 h-5 mr-2" />
          Betting is globally disabled. Enable it above to create or manage betting rounds.
        </div>
      ) : (
        !apiAvailable && (
          <div className="p-3 bg-warning/15 dark:bg-warning/20 border border-warning/40 dark:border-warning/40 text-sm text-vnext-amber rounded-md flex items-center">
            <ShieldExclamationIcon className="w-5 h-5 mr-2" />
      ...
      <section>
        <h2 className="text-2xl font-semibold text-foreground mb-4">
          {translate('adminDashboard.openRoundsTitle', { count: openRounds.length })}
        </h2>
        {openRounds.length === 0 && !isLoading ? (
          <EmptyState
            icon={<PlusCircleIcon />}
            title={translate('adminDashboard.noOpenRounds')}
            action={
              <Button onClick={() => setIsCreateModalOpen(true)} disabled={!isBettingEnabled} size="sm">
      ...
      <section>
        <h2 className="text-2xl font-semibold text-foreground mb-4">
          {translate('adminDashboard.closedRoundsTitle', { count: closedRounds.length })}
        </h2>
        {closedRounds.length === 0 && !isLoading ? (
          <EmptyState icon={<ListBulletIcon />} title={translate('adminDashboard.noClosedRounds')} />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
